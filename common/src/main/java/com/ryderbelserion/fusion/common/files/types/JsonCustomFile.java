...
import com.ryderbelserion.fusion.common.api.interfaces.files.IConfigFile;
...
/**
 * Loads, reloads, or saves existing files built using Configurate Gson.
 */
public class JsonCustomFile extends IConfigFile<JsonCustomFile, BasicConfigurationNode, GsonConfigurationLoader> {

    /**
     * Constructs a {@code JsonCustomFile} with the specified file path, actions, and loader.
     *
    ...
    public JsonCustomFile(@NotNull final Path path, @NotNull final List<FileAction> actions, @Nullable final UnaryOperator<ConfigurationOptions> options) {
        super(path, actions, GsonConfigurationLoader.builder().defaultOptions(options != null ? options : configurationOptions -> configurationOptions).indent(2).path(path).build());
    }
     ...
     */
    @Override
    public @NotNull final BasicConfigurationNode loadConfig() throws ConfigurateException {
